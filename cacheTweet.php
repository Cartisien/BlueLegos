...
require_once('blocked_names.php');

$negative_strings = array('suck', 'fail', 'hate', 'horrible', 'terrible', 'bad', 'awful', 'shitty', 'shit', 'crap', 'who', 'cares', 'asshole', 'shit', 'epic', 'crash', 'crashing', 'flash', 'js', 'javascript', 'bluelego', 'dealsplus', 'Sick', 'rebooting', 'cant', 'wont', 'hurt','not', 'Waste', 'Money', 'Crashes', 'Tried', 'Useless', 'Nothing', 'Paid', 'Open', 'Deleted', 'Downloaded', 'Didnt', 'Says', 'Stupid', 'Anything', 'Actually', 'Account', 'Bought', 'Apple', 'Already');
...
for($a=0; $a<count($hashtags); $a++)
{
	...
	for($i = 1; $i<=$page_limit; $i++)
	{
		...
		{		
			...
			$query = "SELECT * FROM tweet_".$hashtag." WHERE text=\"$text\"";
			$result = mysql_query($query);
			$num = @mysql_num_rows($result); 
			if ($num == 0) 
			{ 
				//this text doesn't exist, probably a new one
